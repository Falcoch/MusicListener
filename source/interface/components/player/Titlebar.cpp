...
namespace tock::interface::player {
    Titlebar::Titlebar(Player * parent, float height)
    : Component(parent), _height(height) {}
    
    void Titlebar::render(double time) {
        float offset = static_cast<float>(this->parent()->window().width()) / 10.f;

        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));
        ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0, 0));

        if(ImGui::Button("Open", ImVec2(offset * 1.5f, this->_height))) {
            bool first = this->parent()->currentTrack() == nullptr;
            this->parent()->load();
            if(!this->parent()->player().isPausing() || first) {
                this->parent()->play();
            }
        }
        ...
        if(ImGui::Button("-", ImVec2(offset, this->_height))) {
            this->parent()->parent()->window().minimize();
        }
